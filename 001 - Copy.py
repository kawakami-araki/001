    ...
    while i < 5:
        try:
            ...
            if data_list == []:
                ...
                data_list = data_list + np.array(data)
        i += 1
        print(data_list)
    column_list = [
        'AEB','VDC','ABS','TCS_Activate','HDC','EPB','cycle','engine','BrakeOverHeat','Front_angle_radar','ESP','TCS_Off','FLWheelRotated','RLWheelRotated','FRWheelRotated','RRWheelRotated','NoBrakeForce','Over_ride','GearLvl','Door','SeatBelt'
    ]
    data_list = data_list.T.tolist()
    all_data = pd.DataFrame(data_list, columns=column_list)
    print(all_data)
