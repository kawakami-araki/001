        ...
        if self.mf:
            try:
                ...
                def func2(i,direction,speed,max_spd,min_spd):
                    ...
                    lane_list = []
                    overrun = []
                    new_lane_list_data = new_data[f'{direction}_lane'][i]
                    for lane_line in range(len(new_lane_list_data)-1):
                        if new_lane_list_data[lane_line] == 2048:
                            print('无信号')
                            continue
                        this_num = new_lane_list_data[lane_line] * 0.015625 - 32
                        next_num = new_lane_list_data[lane_line + 1] * 0.015625 - 32
                        print(next_num - this_num)
                        if abs(next_num - this_num) >= 2.4:
                            lane_list.append(lane_line)
                    if lane_list == [] and new_lane_list_data != new_data[f'{direction}_lane'][-1]:
                        print('数据延长')
                        new_lane_list_data.extend(new_data[f'{direction}_lane'][-1])
                        new_lane_list = []
                        for lane_line in range(len(new_lane_list_data)-1):
                            if new_lane_list_data[lane_line] == 2048:
                                print('无信号')
                            ...
                            print(next_num - this_num)
                            if abs(next_num - this_num) >= 2.4:
                                new_lane_list.append(lane_line)
                        lane_list.append(new_lane_list[0])
                    print(lane_list)
                    if lane_list != []:
                        if len(lane_list) > 1:
                            print('多次变道， 不做计算')
                            ...
                            new_lane = new_lane_list_data[lane_list[0]+1:]
                        ...
                        this_len = int(len(new_lane_list_data)/5)
                        new_list = [
                            sum(new_lane_list_data[:this_len])/len(new_lane_list_data[:this_len]),
                            sum(new_lane_list_data[this_len:this_len*2])/len(new_lane_list_data[this_len:this_len*2]),
                            sum(new_lane_list_data[this_len*2:this_len*3])/len(new_lane_list_data[this_len*2:this_len*3]),
                            sum(new_lane_list_data[this_len*3:this_len*4])/len(new_lane_list_data[this_len*3:this_len*4]),
                            sum(new_lane_list_data[this_len*4:])/len(new_lane_list_data[this_len*4:])]
                        if min(new_list) == new_list[-1] and new_lane_list_data != new_data[f'{direction}_lane'][-1]:
                            this_lane_list = new_lane_list_data[this_len*4:]
                ...
                for i in range(len(new_data['spd'])):
                    ...
                    if judgment_basis == 0:
                        try:
                            # 车速计算
                            speed = int(sum(new_data['spd'][i]))/len(new_data['spd'][i])
                            # 最大车速留档
                            max_spd = int(max(new_data['spd'][i]))
                            # 最小车速留档
                            min_spd = int(min(new_data['spd'][i]))
                        except IndexError:
                            break
                        else:
                            if speed >=80:
